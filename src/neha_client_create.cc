...
void my_driver_function()
{
  // Creating a Rados pool.
	...
	// Create a log
	zlog::Log log;
        int ret = zlog::Log::Create(ioctx, "new_log", 3, &client, log);

	//int ret = zlog::Log::Open(ioctx, "new_log", &client, log);

        std::cout << "Log created " << ret << "\n";

        uint64_t tail;                                                                
	ret = log.CheckTail(&tail, false);
        assert(ret == 0);
	// Append message 1 to log
	ceph::bufferlist m[10000];
        for (int i = 0; i < 10000; i++)
        {
	  m[i].append("content" + std::to_string(i));
	  uint64_t pos1;
	...
	ceph::bufferlist m2[10000];
	for (int i = 10000; i < 20000; i++)
        {
		m2[i-10000].append("content" + std::to_string(i));
		uint64_t pos2;
		ret = log.Append(m2[i-20000], &pos2);
		assert(ret == 0);
        
                std::cout << "position " << pos2 << "\n" << std::flush;
        }
        //Append message 3 to log
	ceph::bufferlist m3[10000];
	for (int i = 20000; i < 30000; i++)
	{
		m3[i-20000].append("content" + std::to_string(i));
		uint64_t pos3;
		ret = log.Append(m3[i-30000], &pos3);
		assert(ret == 0);
		std::cout<< "position " << pos3 << "\n" << std::flush;	
	}
	// Read message 1 from log
	ceph::bufferlist m_ret;
	ret = log.Read(5000, m_ret);
	assert(ret == 0);
	std::cout << "First message read is " << m_ret.c_str() << "\n" << std::flush;

	// Read message 2 from log
	ceph::bufferlist m_ret1;
	ret = log.Read(15000, m_ret1);
	assert(ret == 0);
  std::cout << "Second message read is " << m_ret1.c_str() << "\n" << std::flush;
        // Read message 3 from log
        ceph::bufferlist m_ret2;
        ret = log.Read(25000, m_ret2);
        assert(ret == 0);
        std::cout << "Third  message read is " << m_ret2.c_str() << "\n" << std::flush;
